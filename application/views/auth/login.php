    ...
    <div id="auth">
        <div class="row h-100">
            <div class="col-lg-5 col-12">
                <div id="auth-left">
                    <div class="auth-logo">
                        <a href=""><img src="<?= base_url() . 'dist'; ?>/assets/compiled/svg/iconwafood.svg" alt="Logo" /></a>
                    </div>
                    <h1 class="auth-title">Masuk</h1>
                    <?= $this->session->flashdata('message'); ?>
                    <form method="post" action="<?= base_url('auth'); ?>">
                        <div class="form-group position-relative has-icon-left mb-4">
                            <input type="text" name="username" class="form-control form-control-xl <?= form_error('username') ? 'is-invalid' : ''; ?>" placeholder="Nama Pengguna" autocomplete="off" />
                            <div class="form-control-icon">
                                <i class="bi bi-person"></i>
                            </div>
                            <?= form_error('username', '<div class="invalid-feedback">
                                <i class="bx bx-radio-circle"></i>', '</div>'); ?>
                        </div>
                        <div class="form-group position-relative has-icon-left mb-4">
                            <input type="password" name="password" class="form-control form-control-xl <?= form_error('password') ? 'is-invalid' : ''; ?>" placeholder="Kata Sandi" autocomplete="off" />
                            <div class="form-control-icon">
                                <i class="bi bi-shield-lock"></i>
                            </div>
                            <?= form_error('password', ' <div class="invalid-feedback">
                                <i class="bx bx-radio-circle"></i>', '</div>'); ?>
                        </div>
